...
func (d *Dao) GetTPlanByMessageID(ctx context.Context, messageID int) (*model.TPlan, error) {
	var tPlan *model.TPlan
	result := d.orm.WithContext(ctx).Model(&model.TPlan{}).Where("message_id = ?", messageID).First(&tPlan)
...
func (d *Dao) DeleteTPlanByMessageID(ctx context.Context, messageID, userID int) error {
