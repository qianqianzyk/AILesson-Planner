...
func (d *Dao) StoreMessage(messages []model.ConversationMessage) (int, error) {
	tx := d.orm.Begin()
	var aiMessageIDs int
	for _, msg := range messages {
		...
		if msg.Role == "ai" {
			aiMessageIDs = int(msg.ID)
		}
	}
	if err := tx.Commit().Error; err != nil {
		return 0, err
	}
	return aiMessageIDs, nil
}
...
func (d *Dao) GetMessageList(ctx context.Context, sessionID, pageNum, pageSize int) ([]model.ConversationMessage, *int64, error) {
	var messages []model.ConversationMessage
	var sum int64
	err := d.orm.WithContext(ctx).Model(model.ConversationMessage{}).Where("session_id = ?", sessionID).Order("created_at DESC, role DESC").Count(&sum).Limit(pageSize).Offset((pageNum - 1) * pageSize).Find(&messages).Error
...
func (d *Dao) UpdateMessageByID(id int, newMessage string) error {
	if err := d.orm.Model(&model.ConversationMessage{}).
		Where("id = ?", id).
...
func (d *Dao) GetMessageByID(id int) (*model.ConversationMessage, error) {
	var message *model.ConversationMessage
	result := d.orm.Model(&model.ConversationMessage{}).
		Where("id = ?", id).
		First(&message)
