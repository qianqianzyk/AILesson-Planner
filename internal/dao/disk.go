...
func (d *Dao) GetDirectoryStructure(ctx context.Context, userID, parentID int) ([]model.File, error) {
	var files []model.File
	result := d.orm.WithContext(ctx).Model(&model.File{}).
		Where("user_id = ? AND parent_id = ? AND is_dir = ?", userID, parentID, true).
		Find(&files)
	return files, result.Error
}

func (d *Dao) GetRootDirectory(ctx context.Context, userID int) ([]*model.File, error) {
	var files []*model.File
	result := d.orm.WithContext(ctx).Model(&model.File{}).Where("user_id = ? AND parent_id = ?", userID, 0).
		Find(&files)
	return files, result.Error
}

func (d *Dao) GetFileList(ctx context.Context, parentID int) ([]model.File, *int64, error) {
	var files []model.File
	var sum int64
	result := d.orm.WithContext(ctx).Model(&model.File{}).Where("parent_id = ?", parentID).
		Count(&sum).Find(&files)
	return files, &sum, result.Error
}
...
func (d *Dao) EnsureCourseResourceFolder(ctx context.Context, userID int, courseName string, fileType int) (int, string, error) {
	var courseFolder, subFolder model.File

	err := d.orm.WithContext(ctx).Where("user_id = ? AND parent_id = 0 AND name = ?", userID, courseName).First(&courseFolder).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		courseFolder = model.File{
			UserID:    userID,
			Name:      courseName,
			Path:      "/",
			Size:      0,
		...
		if err := d.orm.WithContext(ctx).Create(&courseFolder).Error; err != nil {
			return 0, "", err
		}
	} else if err != nil {
		return 0, "", err
	...
	if fileType == 3 {
		...
		return 0, "", errors.New("无效的 fileType")
	}

	err = d.orm.WithContext(ctx).Where("user_id = ? AND parent_id = ? AND name = ?", userID, courseFolder.ID, subFolderName).First(&subFolder).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		subFolder = model.File{
			UserID:    userID,
			Name:      subFolderName,
			Path:      "/" + courseName,
			Size:      0,
			FileType:  "文件夹",
			FType:     0,
			ParentID:  int(courseFolder.ID),
			IsDir:     true,
			IsCollect: false,
			CreatedAt: time.Now(),
			UpdatedAt: time.Now(),
		}
		if err := d.orm.WithContext(ctx).Create(&subFolder).Error; err != nil {
			return 0, "", err
		}
	} else if err != nil {
		return 0, "", err
	...
	return int(subFolder.ID), "/" + courseName + "/" + subFolderName, nil
}
