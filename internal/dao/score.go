...
func (d *Dao) GetStudentTranscripts(ctx context.Context, academicYear, name, studentID string, academicTerm int, class string) ([]model.StudentTranscripts, error) {
	...
	err := d.orm.WithContext(ctx).
		...
		Where("s.class = ? AND s.name LIKE ? AND s.student_id LIKE ?", class, "%"+name+"%", "%"+studentID+"%").
...
func (d *Dao) GetStudentCourses(ctx context.Context, studentID string) ([]model.StudentScores, error) {
	...
	err := d.orm.WithContext(ctx).
		...
		Order(`
		CASE 
			WHEN courses.course_name = '中国近代史' THEN 0
			ELSE 1 
		END,
		CAST(SUBSTRING(courses.academic_year, 1, 4) AS SIGNED) DESC,
		courses.academic_term DESC
		`).
