...
func DeleteStudents(studentIDs []string, courseID int) error {
	err := d.DeleteStudents(ctx, studentIDs, courseID)
	return err
}

func GetStudentByStudentIDAndCourseID(studentID string, courseID int) (*model.Score, error) {
	score, err := d.GetStudentByStudentIDAndCourseID(ctx, studentID, courseID)
	return score, err
}
