...
func SaveMessage(sessionID int, messages []model.ConversationMessage) (int, error) {
	messageID, err := d.StoreMessage(messages)
	if err != nil {
		return 0, err
	}
	return messageID, d.UpdateSessionUpdatedAt(sessionID)
}
...
func GetAnswerTextByMoonshot(chatEndpoint, question string) (map[string]string, error) {
...
func UpdateMessageByID(id int, newMessage string) error {
	err := d.UpdateMessageByID(id, newMessage)
	return err
}

func GetMessageByID(id int) (*model.ConversationMessage, error) {
	message, err := d.GetMessageByID(id)
	return message, err
}
