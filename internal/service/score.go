...
func ExportStudentScores(courseName, courseClass string, scores []model.StudentScore) (string, error) {
	// 创建一个新的Excel文件
	f := excelize.NewFile()
	sheetName := "Sheet1"
	// 设置标题样式（加粗 + 居中）
	...
	// 设置第一行标题
	f.SetCellValue(sheetName, "A1", courseName+" "+courseClass+" 成绩")
	...
	// 生成 MinIO 存储路径
	objectKey := "scores/" + courseName + "_" + courseClass + "_成绩.xlsx"
	// 调用 MinIO 上传
...
func GetStudentTranscripts(academicYear, name, studentID string, academicTerm int, class string) ([]model.StudentTranscripts, error) {
	transcripts, err := d.GetStudentTranscripts(ctx, academicYear, name, studentID, academicTerm, class)
