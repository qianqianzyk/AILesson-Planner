...
func GetDirectoryStructure(userID, parentID int) ([]model.File, error) {
	files, err := d.GetDirectoryStructure(ctx, userID, parentID)
	return files, err
}

func GetRootDirectory(userID int) ([]*model.File, error) {
	files, err := d.GetRootDirectory(ctx, userID)
	return files, err
}

func GetFileListByParentID(parentID int) ([]model.File, *int64, error) {
	files, totalSize, err := d.GetFileList(ctx, parentID)
	return files, totalSize, err
}
...
func EnsureCourseResourceFolder(userID int, courseName string, fileType int) (int, string, error) {
	id, path, err := d.EnsureCourseResourceFolder(ctx, userID, courseName, fileType)
	return id, path, err
}
