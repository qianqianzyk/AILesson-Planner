...
func SendStudentTranscripts(fromEmail, sendEmail, sendKey string) error {
	...
	body := "你好，浅浅同学：\n  这是你2025-2026学年第1学期的期末成绩单，请注意查收！"
	m.SetBody("text/plain", body)
	// 附件Url
	attachmentURL := "https://disk.qianqianzyk.top/aihelper/transcripts/2025/xlsx/20250001_2025-2026_1_期末成绩单.xlsx"
	// 下载附件到临时文件
	...
	// 读取文件内容
	tempFile, err := os.CreateTemp("", "20250001_2025-2026_1_期末成绩单_temp.xlsx")
	if err != nil {
	...
	tempFile.Close()
	// 添加附件
	m.Attach(tempFile.Name(), gomail.Rename("20250001_2025-2026_1_期末成绩单.xlsx"))
	// 发送邮件
...
func SendStudentWrongProblem(fromEmail, sendEmail, sendKey string) error {
	m := gomail.NewMessage()
	m.SetHeader("From", fromEmail)
	m.SetHeader("To", sendEmail)
	m.SetHeader("Subject", "[XXXXXX 中国近代史 第1章 错题重做提醒]")
	body := "你好，浅浅同学：\n  这是老师发给你的中国近代史课程的错题，请及时重做并巩固！"
	m.SetBody("text/plain", body)
	// 附件Url
	attachmentURL := "https://disk.qianqianzyk.top/aihelper/scores/2025/docx/浅浅_中国近代史_第1章_错题.docx"
	// 下载附件到临时文件
	resp, err := http.Get(attachmentURL)
	if err != nil {
		return fmt.Errorf("无法下载附件: %w", err)
	...
	defer resp.Body.Close()
	// 读取文件内容
	tempFile, err := os.CreateTemp("", "wrong_problem_*.docx")
	if err != nil {
		return fmt.Errorf("创建临时文件失败: %w", err)
	}
	defer os.Remove(tempFile.Name())
	...
	tempFile.Close()
	// 添加附件
	m.Attach(tempFile.Name(), gomail.Rename("浅浅_中国近代史_第1章_错题.docx"))
	// 发送邮件
	dialer := gomail.NewDialer(SMTPServer, SMTPPort, fromEmail, sendKey)
	if err := dialer.DialAndSend(m); err != nil {
		return fmt.Errorf("发送邮件失败: %w", err)
