...
func (l *SearchShareResourceLogic) SearchShareResource(req *types.SearchShareResourceReq) (resp *types.SearchShareResourceResp, err error) {
	...
	posts, err := service.SearchShareResource(resourceType, service.GetSubjectNameByName(subjectName), keyword)
	if err != nil {
	...
	for _, p := range posts {
		...
		experiencePosts = append(experiencePosts, types.ShareResource{
			...
			CoverImg:    p.CoverImg,
			Title:       p.Title,
			SubjectName: service.GetSubjectNameByID(p.ContentType),
			Content:     p.Content,
			PdfUrl:      p.PdfUrl,
